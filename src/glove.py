...
def create_glove_embedding_matrix(params, tokenizer):
    sorted_words = sorted(tokenizer.word_counts.items(), key=lambda x: x[1], reverse=True)
    top_words = set([word for word, count in sorted_words[:params['num_top_word']]])

    embedding_matrix = np.zeros((len(top_words) + 1, params['embedding_dim']))
    ...
    with open(params['filepath']) as f:
        for line in f:
            ...
            if word in top_words:
                ...
                if idx < len(top_words) + 1:
                    embedding_matrix[idx] = np.array(vector, dtype=np.float32)[:params['embedding_dim']]
...
def build_cnn(params, embedding_matrix=None):
    ...
    if embedding_matrix is not None:
        ...
        x = Embedding(params['vocab_size'], params['embedding_dim'])(inputs)
...
def cnn_mlflow_run(
    ...
    with mlflow.start_run(run_name=name):
        ...
        if glove_params is not None:
            mlflow.log_params(glove_params)
            print('Glove embedding\n')
            embedding_matrix = create_glove_embedding_matrix(glove_params, tokenizer)
            cnn = build_cnn(cnn_params, embedding_matrix)
            nonzero_elements = np.count_nonzero(np.count_nonzero(embedding_matrix, axis=1)) / (glove_params['num_top_word'] + 1)
            mlflow.log_metric("Percentage of covered vocab by embedding",nonzero_elements)
        else:
...
glove_params = {
    'filepath' : 'data/glove/glove.twitter.27B.200d.txt'
    ,'embedding_dim' : 50
    ,'num_top_word' : preprocessing_params['max_words']
}
